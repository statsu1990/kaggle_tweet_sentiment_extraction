        ...
        for data in tqdm(test_loader):
            ...
            for model in models:
                with torch.no_grad():
                    ...
                    start_logits.append(torch.softmax(start_logits, dim=1).cpu().detach().numpy())
                    end_logits.append(torch.softmax(end_logits, dim=1).cpu().detach().numpy())
