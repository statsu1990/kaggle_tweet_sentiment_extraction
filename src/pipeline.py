...
class Model_v1_12_1():
    """
    score 0.726108, lb 0.708
    sub task answering match sentiment and text, p=0.5, batch size=64
    ...
    def train(self, only_val=False):
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df.sentiment)):
            if fold < NUM_FOLD:
                ...
                model, val_score = training.train_model(
                            model, dataloaders_dict, criterion, optimizer, 
                            5, GRAD_ACCUM_STEP, 2, step_scheduler,
                            self.FILENAME_HEAD+'fineturn_', fold, ONLY_VAL)
