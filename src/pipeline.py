...
from data import data_utils, pre_proc
from dataloader import data_loader
from model import tweet_model, loss
from training import training
from prediction import predicting, pred_utils, post_proc
...
class Model_v1_18_1():
    """
    cv 0.558128 (only posi and nega), lb 
    ...
    Model_v1_18_1 (0.0, 0.01,  0.0001): cv 0.558128
...
class Model_v1_19_0():
    ...
    post processing
    base 0.558128
    postproc_selected_text_v7 0.561152
...
class Model_v1_20_0():
    """
    cv 0.550311 (only posi and nega), lb 
    XLNetQAHead(n_top=5)
    ...
    """
    """
    XLNetQAHead and weight decay

    Model_v1_20_0, XLNetQAHead(n_top=5) : 0.550311
    Model_v1_20_1, XLNetQAHead2(n_top=5) : 0.549920
    Model_v1_20_2, XLNetQAHead3(n_top=5) : 0.549871
    """
    def __init__(self):
        self.set_config()
        return
    ...
    def get_model(self):
        ...
        N_TOP = 5
        ans_idx_head = tweet_model.XLNetQAHead(768, N_TOP)
...
class Model_v1_22_4():
    """
    fork Model_v1_21_0
    cv 0.727401 , 0.727597 with postproc_selected_text_v14, lb 0.713
    cv 0.624711 , 0.628477 with postproc_selected_text_v14 (only nega and posi), lb 0.713
    warmup = 0
    ...
    def train(self, only_val=False):
        ...
        # data
        #preproc_func_for_tr = pre_proc.PreprocSelectedText(post_proc.postproc_selected_text_v14)
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df.sentiment)):
            if fold < NUM_FOLD:
                ...
                dataloaders_dict = data_loader.get_train_val_loaders(train_df, train_idx, val_idx, 
                                                                     ...
                                                                     self.VOCAB_FILE, self.MERGES_FILE, 
                                                                     premake_dataset=self.PREMAKE_DATASET,
                                                                     preproc_func_for_tr=preproc_func_for_tr)
...
class Model_v1_23_0():
    """
    fork Model_v1_22_4
    [modified data]cv 0.567110 , 0.570673 with postproc_selected_text_v14 (only nega and posi), lb 0.
    [original data]cv 0.557255 , 0.560869 with postproc_selected_text_v14 (only nega and posi), lb 0.
    modified train_modified.csv
    ...
    def train(self, only_val=False):
        ...
        EPOCHS = 10 if not ONLY_VAL else 1
        BATCH_SIZE = 32
        WARMUP_EPOCHS = 0
        GRAD_ACCUM_STEP = 1
        LR = 1e-5
        ...
        # data
        train_df = data_utils.get_original_data(is_train=True, modified=self.TRAIN_MODIFIED_CSV)
        if self.TRAIN_ONLY_POSI_NEGA:
            train_df = data_utils.remove_neutral(train_df) # to train only positive and negative
...
class Model_v1_23_1():
    """
    eq Model_v1_18_1
    [modified data]cv 0.567946 , 0.572420 with postproc_selected_text_v14 (only nega and posi), lb 0.
    [original data]cv 0.558128 , 0.562398 with postproc_selected_text_v14 (only nega and posi), lb 0.
